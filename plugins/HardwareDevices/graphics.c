...
NTSTATUS GraphicsQueryAttributes(
    _In_ D3DKMT_HANDLE AdapterHandle,
    _Out_ PGX_ATTRIBUTES Attributes
    )
{
    ...
    if (!NT_SUCCESS(status = GraphicsQueryPropertyString(AdapterHandle, &dxCoreAttributes, &adapterAttributes)))
        if (!NT_SUCCESS(status = GraphicsQueryPropertyString(AdapterHandle, &dxAttributes, &adapterAttributes)))
            return status;
